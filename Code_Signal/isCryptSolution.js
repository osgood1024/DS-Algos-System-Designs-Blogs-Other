...
link:https://app.codesignal.com/interview-practice/task/yM4uWYeQTHzYewW9H/description
...
function isCryptSolution(crypt, solution) {
    ...
    for(let i =0;i<crypt.length;i++){
       let string= crypt[i].split('')
        for(let j=0;j<string.length;j++){
            if(j===0 &&  dict[string[j]]=== '0'){
               return false 
            }
            if(i===0){
                first+=dict[string[j]]
            }else if (i===1){
                second+=dict[string[j]]
            }else if (i===2){
                last+=dict[string[j]]
